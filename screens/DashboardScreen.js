...
function StatPill({ value, label, accent }) {
  ...
  const { statStyles } = getStyles(theme);
  return (
...
function ActionTile({ emoji, label, sub, onPress, variant = "light" }) {
  const { theme } = useContext(ThemeContext);
  const { tileStyles } = getStyles(theme);
...
function ActivityRow({ emoji, title, date, points, negative }) {
  const { theme } = useContext(ThemeContext);
  const { actStyles } = getStyles(theme);
  return (
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={{ opacity: fadeHero, transform: [{ translateY: slideHero }] }}>
          <Pressable style={styles.heroBanner} onPress={() => navigation.navigate("Wallet")}>
            ...
            <View style={styles.statsRow}>
              ...
              <StatPill value="23" label="Deposits" accent={theme.text} />
              <View style={styles.statSep} />
              <StatPill value="4" label="Redeemed" accent={theme.textMuted} />
            </View>
          </Pressable>
        </Animated.View>
        ...
        <Animated.View style={{ opacity: fadeCards }}>
          ...
          <View style={styles.tileRow}>
            <ActionTile emoji="📷" label="Scan & Deposit" sub="Open QR Scanner" variant="lime" onPress={() => navigation.navigate("QRScreen")} />
            <ActionTile emoji="🗺️" label="Find a Bin" sub="Locate Drop-Off" variant="dark" onPress={() => navigation.navigate("EcoMap")} />
          </View>

          <View style={[styles.tileRow, { marginTop: 10 }]}>
            <ActionTile emoji="🎁" label="Redeem" sub="Use Your Points" variant="bone" onPress={() => navigation.navigate("Wallet")} />
            <ActionTile emoji="👤" label="Profile" sub="View and Edit Info" variant="moss" onPress={() => navigation.navigate("Profile")} />
          </View>
        </Animated.View>
